...
def build_model(args):
    model = Sequential()
    model.add(Lambda(lambda x: x/225-0.5, input_shape=INPUT_SHAPE))
    model.add(Conv2D(24, 5, 5, activation='elu', subsample=(2, 2)))
    model.add(Conv2D(36, 5, 5, activation='elu', subsample=(2, 2)))
    model.add(Conv2D(48, 5, 5, activation='elu', subsample=(2, 2)))
    #model.add(Conv2D(48, 5, 5, activation='elu', subsample=(2, 2)))                 #extra layer added
    model.add(Conv2D(64, 3, 3, activation='elu'))
    model.add(Conv2D(64, 3, 3, activation='elu'))
    model.add(Dropout(args.keep_prob))
    model.add(Flatten())
    model.add(Dense(100, activation='elu'))
    model.add(Dense(75, activation='elu'))                                          #extra layer added     
...
def train_model(model, args, X_train, X_valid, y_train, y_valid):
    ...
    plt.suptitle('PilotNet/NVIDIA Model')
    plt.subplot(1, 2, 1)
...
def main():
    ...
    parser.add_argument('-s', help='samples per epoch',     dest='samples_per_epoch', type=int,   default=10100)    #20000
